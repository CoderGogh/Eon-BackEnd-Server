...
> **결과:** API 평균 응답 속도 **약 76% 개선**(500ms -> 120ms) 및 외부 API Rate Limit 회피를 통한 시스템 안정성 확보.
...
## 🛠 Troubleshooting: 성능 병목 해결 및 최적화

프로젝트 개발 과정에서 발생한 기술적 난관과 이를 해결하기 위한 엔지니어링 의사결정 과정입니다.

### 1. PostGIS 공간 쿼리 성능 저하 및 캐시 효율성 문제
* **문제**: 유저의 미세한 위치 변화(위경도 소수점 7~8자리)마다 새로운 캐시가 생성되어 Redis 메모리 사용량이 급증하고, 캐시 히트율(Hit Rate)이 현저히 낮아지는 문제 발생.
* **원인**: 위경도의 미세한 차이를 모두 개별 요청으로 인식하여 `ST_DWithin` 쿼리가 매번 실행됨.
* **해결**: 
    - **좌표 정규화(Rounding Strategy)**: 요청받은 좌표를 특정 소수점 자리(`CACHE_COORD_ROUND_DECIMALS`)에서 반올림하여 '버킷(Bucket)'화함.
    - **결과**: 동일 반경 내 요청을 하나의 캐시 키로 통합하여 **캐시 히트율을 약 40% 이상 끌어올렸으며**, DB 부하를 획기적으로 줄임.

### 2. 외부 API(KEPCO)의 높은 지연 시간(Latency) 및 불안정성
* **문제**: 실시간 충전기 상태를 가져오기 위해 KEPCO API를 호출할 때, 평균 응답 시간이 500ms를 초과하며 간헐적인 타임아웃 발생.
* **원인**: 외부 인프라의 응답 속도에 내부 서비스 가용성이 종속되는 구조적 한계.
* **해결**:
    - **Multi-Tier Caching**: 정적 정보(L2)와 실시간 정보(L3)의 TTL을 분리하여 관리.
    - **Async I/O & Timeout Control**: `httpx` 비동기 클라이언트에 엄격한 타임아웃을 설정하고, API 실패 시 최신 캐시 데이터를 반환하는 **Fallback 전략** 구현.
    - **성과**: 전체 API 응답 속도를 **500ms에서 120ms로 약 76% 개선**하고, 외부 API 장애 시에도 서비스 연속성 유지.

### 3. 대량의 데이터 처리 시 커넥션 풀 고갈
* **문제**: 부하 테스트(Locust) 진행 중 대량의 동시 요청 발생 시 DB Connection Pool이 고갈되어 응답 실패율 상승.
* **해결**: 
    - **Connection 관리 최적화**: SQLAlchemy의 `AsyncSession` 생명주기를 FastAPI의 `Depends`를 통해 엄격히 관리하여 세션 누수 방지.
    - **Nginx Rate Limiting**: 비정상적인 반복 요청에 대해 레이어별 제한을 두어 백엔드 서버 도달 전 차단.
